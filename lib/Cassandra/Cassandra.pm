    ...
    $xfer += $output->writeFieldBegin('success', TType::MAP, 0);
    {
      $xfer += $output->writeMapBegin(TType::STRING, TType::LIST, scalar(keys %{$self->{success}}));
      {
        while( my ($kiter273,$viter274) = each %{$self->{success}}) 
        {
          ...
          {
            $xfer += $output->writeListBegin(TType::STRING, scalar(@{${viter274}}));
            {
              foreach my $iter275 (@{${viter274}}) 
              {
      ...
      /^0$/ && do{      if ($ftype == TType::LIST) {
        {
          ...
          {
            my $elem288 = undef;
            $elem288 = new Cassandra::TokenRange();
...
sub write {
  ...
  if (defined $self->{values}) {
    $xfer += $output->writeFieldBegin('values', TType::LIST, 2);
    {
      ...
      {
        foreach my $iter312 (@{$self->{values}}) 
        {
