      ...
      /^4$/ && do{      if ($ftype == TType::LIST) {
        {
          ...
          for (my $_i164 = 0; $_i164 < $_size160; ++$_i164)
          {
            ...
            push(@{$self->{variable_names}},$elem165);
          }
          $xfer += $input->readListEnd();
        }
      } else {
...
sub write {
  ...
  if (defined $self->{variable_types}) {
    $xfer += $output->writeFieldBegin('variable_types', TType::LIST, 3);
    {
      ...
      {
        foreach my $iter166 (@{$self->{variable_types}}) 
        {
  ...
  if (defined $self->{variable_names}) {
    $xfer += $output->writeFieldBegin('variable_names', TType::LIST, 4);
    {
      $xfer += $output->writeListBegin(TType::STRING, scalar(@{$self->{variable_names}}));
      {
        foreach my $iter167 (@{$self->{variable_names}}) 
        {
